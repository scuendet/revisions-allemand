...
import {
  computeVocabSmartScore,
  filterVocabResultsByMode,
...
export async function POST(req: NextRequest) {
  ...
  const scoped = filterVocabResultsByMode(results, mode)
  ...
  const weightW = (w: GermanVocabWord) =>
    computeVocabSmartScore(historyByVocab.get(w.id) ?? [], nowMs).weight

  const annotate = (w: GermanVocabWord) => {
    const h = historyByVocab.get(w.id) ?? []
    const { weight, bullets } = computeVocabSmartScore(h, nowMs)
    return {
      id: w.id,
      unit: w.unit,
  ...
  const randomDetail = (w: GermanVocabWord) => [
    `Exemple : « ${w.french} » → ${w.german.split(',')[0]?.trim() ?? w.german}`,
  ]
  ...
  if (!smart || !hasUser) {
    const exampleDrawRaw = pickGermanVocabQuestions({
      filtered,
      count: countArg,
      smart: false,
      userId: null,

...
export async function POST(req: NextRequest) {
  ...
  const results = await readJson<EnglishResult[]>(`english-results-${user_id}.json`, [])
  const entry: EnglishResult = {
    id: nextId(results),
    vocab_id,
    mode,
    correct: correct ? 1 : 0,
  ...
  await writeJson(`english-results-${user_id}.json`, [...results, entry])

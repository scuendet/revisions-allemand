...
export default function QuizPage() {
  ...
  async function handleFlashcardAnswer(correct: boolean) {
    ...
    setResults(prev => [...prev, { vocabId: current.id, correct, german: current.german, french: current.french }])
  ...
  function handleTypingSubmit() {
    ...
    const expected = normalizeAnswer(current.german)
  ...
  function startRecording() {
    ...
    debugLog('Creating SpeechRecognition (de-DE)')
    ...
    // Start SpeechRecognition FIRST — it gets mic priority.
  ...
  function stopRecording() {
    ...
    recognitionRef.current?.stop()
    // MediaRecorder stops in recognition.onend
  }
              ...
              <div className="space-y-2">
                {missedItems.map((r, i) => (
                  <div key={i} className="flex items-center justify-between text-sm p-2 rounded-lg bg-rose-50">
                    ...
                    <span className="font-semibold text-primary">{r.german}</span>
                  </div>
                ))}
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col p-6">
      <div className="w-full max-w-lg mx-auto flex-1 flex flex-col">
        <QuizChrome
          backLabel="← Menu"
          onBack={() => router.push(`/${userId}/german`)}
        ...
        {mode === 'flashcard' && (
          <div className="flex-1 flex flex-col gap-4">
            {/* Card — fixed height, buttons live outside */}
            <div className="card-flip-container h-56">
              <div className={`card-inner h-full ${flipped ? 'flipped' : ''}`}>
                ...
                <div className="card-back bg-primary rounded-2xl shadow-md flex flex-col items-center justify-center p-8 overflow-hidden">
                  <p className="text-xs font-semibold text-accent/80 uppercase tracking-wide mb-3">Allemand</p>
                  <p className="text-2xl font-bold text-white text-center leading-snug">{current.german}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-8 mb-4">
              ...
              {typingResult?.shown && (
                <div className={`mt-4 p-3 rounded-xl text-sm font-semibold ${typingResult.correct ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
                  {typingResult.correct ? '✅ Correct !' : `❌ La bonne réponse était : ${current.german}`}
                </div>
              )}
            </div>
        ...
        {mode === 'audio' && (
          <div className="flex-1 flex flex-col gap-4">
            <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-8 flex flex-col items-center">
              {/* French prompt with replay button */}
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Français</p>
              ...
              {audioResult && (
                <div className={`w-full p-4 rounded-xl ${audioResult.correct ? 'bg-emerald-50' : 'bg-rose-50'}`}>
                  ...
                  {audioResult.correct ? (
                    ...
                    <p className="text-rose-700 font-semibold">❌ La bonne réponse était : <span className="font-bold">{current.german}</span></p>
                  )}
                </div>
              )}
            </div>
            ...
            {/* Debug log — always visible during audio mode */}

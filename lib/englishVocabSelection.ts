...
export interface EnglishResultRow {
  id: number
  vocab_id: number
  mode: string
...
function isCorrect(value: EnglishResultRow['correct']): boolean {
  return value === 1 || value === true
}

export function filterEnglishResultsByMode(results: EnglishResultRow[], mode?: string): EnglishResultRow[] {
...
export function computeEnglishSmartScore(
  history: EnglishResultRow[],
  nowMs: number,
...
function randomPick(filtered: EnglishVocabWord[], count: number) {
  return shuffle(filtered).slice(0, count)
}

export function pickEnglishVocabQuestions(params: {
  filtered: EnglishVocabWord[]
  count: number | 'all'
  smart: boolean
  userId: number | null
  mode?: string
  results: EnglishResultRow[]
}): EnglishVocabWord[] {
  ...
  const relevant = filterEnglishResultsByMode(results, mode)
  ...
  const weightItem = (item: EnglishVocabWord) =>
    computeEnglishSmartScore(historyByVocab.get(item.id) ?? [], nowMs).weight
